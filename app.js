...
function checkAuth() {
  ...
  if (localStorage.getItem('avp_auth') !== '1') {
...
async function initApp() {
  ...
  // Verifica status WhatsApp a cada 30 segundos
  setInterval(checkWppSidebarStatus, 30000);
...
async function loadData() {
  // Carrega local primeiro — app nunca fica em branco
  loadFromStorage();
  
  if (!SHEETS_URL) return;

  // Cache inteligente — só busca do Sheets se passaram 5 minutos
  const lastFetch = parseInt(localStorage.getItem('lastSheetsFetch') || '0');
  const now = Date.now();
  const FIVE_MINUTES = 5 * 60 * 1000;
  
  if (clients.length > 0 && (now - lastFetch) < FIVE_MINUTES) {
    return; // usa cache local
  }
  ...
  try {
    ...
    if (json.status === 'ok' && Array.isArray(json.clients) && json.clients.length > 0) {
      ...
      localStorage.setItem('lastSheetsFetch', Date.now().toString());
      // Rerenderiza com dados do Sheets
      renderDashboard();
      renderContactsDay();
      renderClients();
      renderRanking();
    ...
    // Sheets falhou ou timeout — mantém dados locais
  }
}
...
async function syncToSheets() {
  if (!SHEETS_URL) return;
  try {
    // Usa text/plain para evitar CORS preflight no Apps Script
...
// ---- WARM DISPATCH ----
function previewWarmDispatch() {
  const warm = clients.filter(c => {
    const d = daysSince(c.lastContact);
    return d >= 8 && d <= 15;
  });
  const withWpp = warm.filter(c => c.whatsapp);
  const withoutWpp = warm.filter(c => !c.whatsapp);
  document.getElementById('warmDispatchStatus').textContent = `✅ ${withWpp.length} mornos prontos · ⚠️ ${withoutWpp.length} sem WhatsApp`;
  showToast(`🌤️ ${withWpp.length} clientes mornos para enviar`);
}

async function startWarmDispatch() {
  const warm = clients.filter(c => {
    const d = daysSince(c.lastContact);
    return d >= 8 && d <= 15 && c.whatsapp;
  });

  if (warm.length === 0) {
    showToast('⚠️ Nenhum cliente morno com WhatsApp!', 'error');
    return;
  }

  // Reusa a fila do disparo normal
  dispatchRunning = true;
  dispatchQueue = warm;
  dispatchIndex = 0;
  window.failedDispatch = [];

  document.getElementById('warmDispatchStatus').textContent = `📤 Iniciando disparo de ${warm.length} clientes mornos...`;
  showToast(`🌤️ Iniciando disparo para ${warm.length} mornos!`);
...
async function checkWppSidebarStatus() {
  ...
  try {
    ...
    // Evolution API v2 pode retornar em formatos diferentes
    let connected = false;
    if (Array.isArray(data)) {
      const instance = data.find(i => 
    ...
    els.forEach(id => {
      ...
      if (!el) return;
      el.innerHTML = connected ? '🟢 WhatsApp conectado' : '🔴 WhatsApp desconectado';
      el.style.color = connected ? '#00c853' : '#ef4444';
    });

    const wppStatus = document.getElementById('whatsappStatus');
    if (wppStatus) {
      wppStatus.innerHTML = connected 
        ? '<span style="color:var(--success)">✅ WhatsApp conectado!</span>'
        : '<span style="color:var(--hot)">❌ WhatsApp desconectado</span>';
    }

  } catch(e) {
    els.forEach(id => {
      const el = document.getElementById(id);
      if (el) { el.innerHTML = '🔴 WhatsApp desconectado'; el.style.color = '#ef4444'; }
    });
  }
}
...
function filterByPurchase(type) {
  navigate('clients');
  const sel = document.getElementById('filterTemp');
  if (sel) sel.value = 'purchase_' + type;
  renderClients();
}
...
function renderClients() {
  clientsPage = 1; // reset page on new filter/search
  ...
  let filtered = clients.filter(c => {
    ...
    const matchFilter =
      ...
      filter === 'purchase_recent' ? (c.lastPurchase && daysSince(c.lastPurchase) <= 7) :
      filter === 'purchase_medium' ? (c.lastPurchase && daysSince(c.lastPurchase) > 7 && daysSince(c.lastPurchase) <= 30) :
      filter === 'purchase_old' ? (!c.lastPurchase || daysSince(c.lastPurchase) > 30) : true;
  ...
  if (filtered.length === 0) {
    ...
    const paginated = filtered.slice(0, clientsPage * CLIENTS_PER_PAGE);
    const hasMore = filtered.length > paginated.length;
    
    container.innerHTML = paginated.map(c => renderClientCard(c, false)).join('');
    ...
    if (hasMore) {
      ...
      btn.style = 'width:100%;margin-top:0.75rem;padding:0.75rem;font-size:0.85rem;';
      btn.textContent = `Ver mais (${filtered.length - paginated.length} restantes)`;
      btn.onclick = () => {
        clientsPage++;
        const morePaginated = filtered.slice(0, clientsPage * CLIENTS_PER_PAGE);
        const moreCards = morePaginated.slice(paginated.length).map(c => renderClientCard(c, false)).join('');
        btn.insertAdjacentHTML('beforebegin', moreCards);
        if (filtered.length <= morePaginated.length) btn.remove();
        else btn.textContent = `Ver mais (${filtered.length - morePaginated.length} restantes)`;
      };
      container.after(btn);
    }
  }
...
function markContacted(id) {
  // Abre modal para anotar o que foi discutido
  const c = clients.find(x => x.id === id);
  if (!c) return;
  
  const modal = document.getElementById('contactNoteModal');
  const title = document.getElementById('contactNoteTitle');
  const textarea = document.getElementById('contactNoteText');
  
  title.textContent = c.empresa;
  textarea.value = '';
  modal.style.display = 'flex';
  setTimeout(() => textarea.focus(), 100);
  
  document.getElementById('contactNoteSaveBtn').onclick = () => {
    const note = textarea.value.trim();
    const t = today();
    c.lastContact = t;
    if (!c.history) c.history = [];
    const label = note ? `Contatado: ${note}` : 'Contatado';
    c.history.unshift({ type: 'contact', date: t, label });
    if (note) {
      const obsLine = `📞 ${formatDate(t)}: ${note}`;
      c.obs = c.obs ? c.obs + '\n' + obsLine : obsLine;
    }
    saveToStorage();
...
function openWhatsApp(number, name) {
  ...
  const msgs = [
    `${saudacao}! Tudo bem por aí? 😊 Passando para saber se posso ajudar com algo esta semana. Tenho novidades e queria compartilhar com você antes! Me fala o que está precisando.`,
    `${saudacao}! Vim dar um alô e ver como as coisas estão por aí 👋 Alguma novidade? Estou à disposição para ajudar no que precisar!`,
    `${saudacao}! Faz um tempo que não conversamos 😊 Passando para não sumir e ver se posso ser útil. Me conta como estão as coisas!`,
    `${saudacao}! Só passando para checar se está tudo certo por aí 😄 Qualquer coisa que precisar estou aqui! O que posso fazer por você?`,
    `${saudacao}! Vim dar um alô rápido 👋 Tem alguma novidade por aí? Me conta que vejo o que posso fazer!`,
    `${saudacao}! Passando para não sumir! rsrs 😄 Alguma coisa que eu possa ajudar essa semana? Me fala o que precisar!`,
  ];
...
async function sendNextMessage() {
  ...
  try {
    // Envia via Google Apps Script (proxy para evitar CORS)
    const proxyUrl = SHEETS_URL
      + '?action=sendWhatsApp'
      + '&number=' + encodeURIComponent(clean)
      + '&message=' + encodeURIComponent(msg);

    const sendRes = await fetch(proxyUrl, { method: 'GET' });
    let sendData = {};
    try { sendData = await sendRes.json(); } catch(e) {}

    if (sendData.status === 'ok' || sendData.code === 200 || sendData.code === 201) {
...
async function checkWhatsAppStatus() {
  ...
  if (statusEl) statusEl.innerHTML = '<span style="color:var(--text2)">🔍 Verificando...</span>';
  await checkWppSidebarStatus();
}
...
async function loadNewLeads() {
  ...
  if (container) container.innerHTML = '<div style="text-align:center;padding:2rem;color:var(--text2)">⏳ Carregando leads...</div>';
  ...
  try {
    ...
    if (!rows.length) {
      ...
      if (empty) {
        ...
        if (p) p.textContent = '📭 Nenhum lead encontrado na base do LeadHunter';
      }
      return;
    }
    ...
    const nlTotal = document.getElementById('nlTotal');
    const nlStats = document.getElementById('newLeadsStats');
    if (nlTotal) nlTotal.textContent = leads.length;
    if (nlStats) nlStats.style.display = 'flex';

    if (!leads.length) {
      if (container) container.innerHTML = '<div class="empty-state"><p>🎉 Todos os leads já foram processados!</p></div>';
      return;
    }

    if (container) container.innerHTML = leads.map(l => renderLeadCard(l)).join('');
...
function renderLeadCard(l) {
  ...
  <div class="client-card" id="lead-${l.placeId.replace(/[^a-z0-9]/gi,'_')}">
    ...
    <div class="client-actions">
      ...
      <button class="btn btn-success" onclick="approveLead(${safeL})">✅ Aprovar</button>
...
function approveLead(l) {
  ...
  const nlApproved = document.getElementById('nlApproved');
  if (nlApproved) nlApproved.textContent = parseInt(nlApproved.textContent || '0') + 1;
...
function discardLead(placeId) {
  ...
  const card = document.getElementById(cardId);
  if (card) card.remove();
  const nlDiscarded = document.getElementById('nlDiscarded');
  if (nlDiscarded) nlDiscarded.textContent = parseInt(nlDiscarded.textContent || '0') + 1;
